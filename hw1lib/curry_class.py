...
class AUDUSD_return(object):
    """
    Currency Class  instance for storing real-time FOREX data.
    The class name shows the currency pair of this class.
    ...
    #求的是变化率的boll指标，从boll指标的定义来看，这么用其实也没有太大问题

    def add_to_running_squared_sum(self,avg):
        """
        Function to add this return to running squared sum.

        :param avg: The average value of the returns.
        :type number: float

        :return: The result of running squared sum.
        :rtype: float
        """
    ...
    def get_avg(self,pop_value):
        """
        Function to calculate the average value of returns.

        :param pop_value: the value in last period which need to be poped.
        :type number: float

        :return: The average value of the returns.
        :rtype: float
        """
        if isnan(self.hist_return) == False:
    ...
    def get_std(self):
        """
        Function to calculate of run sum of std.
    ...
    def get_avg_std(self,pop_value):
        """
        Function to calculate the average of run sum of std.

        :param pop_value: the value in last period which need to be poped.
        :type number: float

        :return: The average std.
        :rtype: float
        """
        if isnan(self.hist_return) == False:
